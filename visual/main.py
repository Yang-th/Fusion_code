...
def main():
    ...
    batchsize_eval = 1
    train_loader, val_loader = load.afew_faces_fan(root_train, list_train, batchsize_train, root_eval, list_eval,
                                                   batchsize_eval)
    ''' Load model '''
    ...
    for epoch in range(args.epochs):
        ...
        acc_epoch, pred_score, feature1024 = val(val_loader, model, at_type)
        feature_array = feature1024.cpu().numpy()
        pred_score_array = pred_score.cpu().numpy()
        ...
        if is_best:
            ...
            np.save('feature/B_dataset_feature256_714'+str(acc_epoch)+'.npy',feature_array)
            np.save('feature/B_dataset_output_714'+str(acc_epoch)+'.npy',pred_score_array)
...
def train(train_loader, model, optimizer, epoch):
    ...
    logger.print(
        ' *Acc@Video {topVideo.avg:.3f}   *Acc@Frame {topframe.avg:.3f} '.format(topVideo=topVideo, topframe=topframe))
...
def val(val_loader, model, at_type):
    ...
    with torch.no_grad():
        ...
        if at_type == 'self_relation-attention':
            pred_score, feature = model(vectors=output_store_fc, vm=weightmean_sourcefc, alphas_from1=output_alpha,
        ...
        return topVideo.avg, pred_score,feature
